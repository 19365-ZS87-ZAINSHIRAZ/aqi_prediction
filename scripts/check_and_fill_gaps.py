"""
Auto Data Gap Detector and Filler
Checks for missing days in historical data and fills gaps
"""
import sys
from pathlib import Path
sys.path.insert(0, str(Path(__file__).parent.parent))

import pandas as pd
import numpy as np
from datetime import datetime, timedelta
from loguru import logger

from src.database.mongodb_operations import MongoDBOperations
from src.analysis.aqi_calculator import AQICalculator
from src.analysis.health_impact import HealthImpactAssessor
from config.settings import MONITORED_CITIES


def check_data_gaps(city: str, db_ops: MongoDBOperations) -> dict:
    """
    Check for missing days in historical data
    
    Returns:
        dict with gap information
    """
    # Get all measurement dates
    measurements = db_ops.measurements.find(
        {'city': city},
        {'timestamp': 1}
    ).sort('timestamp', 1)
    
    dates = [m['timestamp'].date() for m in measurements]
    
    if not dates:
        return {
            'has_data': False,
            'start_date': None,
            'end_date': None,
            'total_days': 0,
            'missing_days': [],
            'coverage': 0
        }
    
    start_date = min(dates)
    end_date = datetime.now().date()
    expected_days = (end_date - start_date).days + 1
    actual_days = len(set(dates))
    
    # Find missing days
    all_expected = {start_date + timedelta(days=i) for i in range(expected_days)}
    existing = set(dates)
    missing = sorted(all_expected - existing)
    
    coverage = (actual_days / expected_days * 100) if expected_days > 0 else 0
    
    return {
        'has_data': True,
        'start_date': start_date,
        'end_date': end_date,
        'total_days': expected_days,
        'actual_days': actual_days,
        'missing_days': missing,
        'coverage': coverage
    }


def generate_missing_day_data(city: str, date: datetime.date, 
                              prev_data: dict, base_params: dict) -> list:
    """
    Generate realistic data for a missing day based on nearby dates
    """
    records = []
    
    # Get seasonal factor
    month = date.month
    if month in [11, 12, 1, 2]:
        seasonal_factor = 1.4
    elif month in [3, 4, 10]:
        seasonal_factor = 1.1
    else:
        seasonal_factor = 0.8
    
    daily_variation = np.random.uniform(0.9, 1.1)
    
    for param, city_values in base_params.items():
        if city not in city_values:
            continue
            
        base_value = city_values[city]
        value = base_value * seasonal_factor * daily_variation
        value += np.random.normal(0, base_value * 0.05)
        value = max(0, value)
        
        unit = 'µg/m³' if param in ['pm25', 'pm10', 'no2', 'so2', 'o3'] else 'mg/m³'
        
        records.append({
            'city': city,
            'parameter': param,
            'value': round(value, 2),
            'unit': unit,
            'timestamp': datetime.combine(date, datetime.min.time()),
            'source': 'Generated',
            'location': f'{city} (Auto-filled)',
            'latitude': MONITORED_CITIES[city]['lat'],
            'longitude': MONITORED_CITIES[city]['lon']
        })
    
    return records


def fill_gaps(city: str, missing_days: list, db_ops: MongoDBOperations):
    """Fill missing days with generated data"""
    
    base_params = {
        'pm25': {'Islamabad': 45, 'Rawalpindi': 50, 'Karachi': 65},
        'pm10': {'Islamabad': 80, 'Rawalpindi': 85, 'Karachi': 110},
        'o3': {'Islamabad': 35, 'Rawalpindi': 38, 'Karachi': 45},
        'no2': {'Islamabad': 25, 'Rawalpindi': 28, 'Karachi': 35},
        'so2': {'Islamabad': 8, 'Rawalpindi': 10, 'Karachi': 12},
        'co': {'Islamabad': 0.6, 'Rawalpindi': 0.7, 'Karachi': 0.9}
    }
    
    aqi_calc = AQICalculator()
    health_assessor = HealthImpactAssessor()
    
    logger.info(f"Filling {len(missing_days)} missing days for {city}")
    
    for missing_date in missing_days:
        # Generate data for this day
        records = generate_missing_day_data(city, missing_date, {}, base_params)
        
        # Save measurements
        df = pd.DataFrame(records)
        db_ops.save_measurements(df)
        
        # Calculate and save AQI
        pollutants = {}
        for record in records:
            if record['parameter'] in ['pm25', 'pm10', 'o3', 'no2', 'so2', 'co']:
                pollutants[record['parameter']] = record['value']
        
        if pollutants:
            aqi_info = aqi_calc.calculate_multi_pollutant_aqi(pollutants)
            
            aqi_data = {
                'timestamp': datetime.combine(missing_date, datetime.min.time()),
                'aqi': aqi_info.get('aqi', 0),
                'category': aqi_info.get('category', 'Unknown'),
                'dominant_pollutant': aqi_info.get('dominant_pollutant', 'pm25'),
                'pollutants': pollutants
            }
            db_ops.save_aqi_history(city, aqi_data)
            
            # Save health impact
            if aqi_info.get('aqi'):
                health_impact = health_assessor.assess_health_impact(city, pollutants, aqi_info['aqi'])
                db_ops.save_health_impact(city, health_impact)
    
    logger.success(f"Filled {len(missing_days)} missing days for {city}")


def main():
    """Check and fill data gaps for all cities"""
    logger.info("=" * 60)
    logger.info("Data Gap Checker & Filler")
    logger.info("=" * 60)
    
    db_ops = MongoDBOperations()
    
    for city in MONITORED_CITIES.keys():
        logger.info(f"\nAnalyzing {city}...")
        
        # Check for gaps
        gap_info = check_data_gaps(city, db_ops)
        
        if not gap_info['has_data']:
            logger.warning(f"  No data found for {city}")
            continue
        
        logger.info(f"  Data range: {gap_info['start_date']} to {gap_info['end_date']}")
        logger.info(f"  Coverage: {gap_info['coverage']:.1f}% ({gap_info['actual_days']}/{gap_info['total_days']} days)")
        
        if gap_info['missing_days']:
            logger.warning(f"  Found {len(gap_info['missing_days'])} missing days")
            logger.info(f"  Missing dates: {gap_info['missing_days'][:5]}{'...' if len(gap_info['missing_days']) > 5 else ''}")
            
            # Fill gaps
            fill_gaps(city, gap_info['missing_days'], db_ops)
        else:
            logger.success(f"  ✓ No gaps - 100% coverage!")
    
    logger.info("\n" + "=" * 60)
    logger.success("Gap check and fill completed!")
    logger.info("=" * 60)


if __name__ == "__main__":
    main()
